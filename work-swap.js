...
function swapShell(title,body){
 ...
 modal.innerHTML=`<div class="nd-pop swap-dialog" role="dialog" aria-modal="true" aria-labelledby="swap-title"><div class="swap-head"><div><b id="swap-title">${title}</b><p>근무 배정 교환 · 상대방 수락 후 관리자 승인</p></div><button aria-label="닫기" onclick="closeWorkSwap()">×</button></div><div class="swap-body">${body}</div><div id="swap-error" role="status" aria-live="polite"></div></div>`;
}
